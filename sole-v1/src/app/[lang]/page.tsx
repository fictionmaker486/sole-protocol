...
export default function Home({ params }: { params: Promise<{ lang: Locale }> }) {
  ...
  return (
    <main className="min-h-screen bg-gray-50 text-black p-6 md:p-12 font-mono overflow-x-hidden">
      ...
      {/* 頂部區域：增加 z-index 與高度防止重疊 */}
      <div className="max-w-4xl mx-auto border-b-8 border-black pb-8 mb-12 flex justify-between items-start relative min-h-[160px]">
        ...
        {/* 右上角 Logo：使用 layoutId 達成「飛入」效果 */}
        <motion.div 
          layoutId="operative-logo"
          className="absolute top-0 right-0 z-50 mt-2"
        >
      ...
      <AnimatePresence mode="wait">
        {!user ? (
          <motion.div
            key="login-view"
            layoutId="main-content-card" // 讓盒子跟儀表板產生形變連結
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, scale: 1.2, rotateX: 10, filter: "blur(10px)" }}
          ...
          >
            <div className="bg-white border-[6px] border-black p-8 shadow-[16px_16px_0px_0px_rgba(0,0,0,1)] text-center">
              ...
              <form onSubmit={handleAuth} className="space-y-6 text-left">
                ...
                <button type="submit" className="w-full bg-black text-white font-black py-5 text-lg hover:bg-red-600 shadow-[8px_8px_0px_0px_rgba(220,38,38,0.3)]">EXECUTE</button>
              </form>
            </div>
          </motion.div>
        ) : (
