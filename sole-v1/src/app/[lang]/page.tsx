...
export default function Home({ params }: { params: Promise<{ lang: Locale }> }) {
  ...
  return (
    <main className="min-h-screen bg-gray-50 text-black p-6 md:p-12 font-mono overflow-x-hidden">
      ...
      {/* 頂部區域 - 增加 z-index 與 min-h 確保 Logo 不會被標題擋住 */}
      <div className="max-w-4xl mx-auto border-b-8 border-black pb-8 mb-12 flex justify-between items-start relative min-h-[150px] z-20">
          ...
          <AnimatePresence mode="wait">
            {!user ? (
              ...
              <motion.img
                ...
                animate={{ opacity: 1, x: 0 }}
                exit={{ opacity: 0, scale: 2, rotateY: 90, filter: "blur(10px)" }}
            ...
            {profiles.map((profile) => (
              <div key={profile.id} className="space-y-10">
                <div className="bg-white border-4 border-black p-8 shadow-[12px_12px_0px_0px_rgba(0,0,0,1)] relative">
                  <div className="bg-black text-white text-[10px] px-2 py-1 font-bold inline-block mb-6 uppercase">Authorized Operative</div>
                  ...
                  <div className="mt-10 border-t-2 pt-4 flex justify-between items-center">
                    ...
                    <span className="font-black text-3xl">{profile.credibility_score} <span className="text-xs text-gray-400">/ 100</span></span>
                  </div>
                </div>
